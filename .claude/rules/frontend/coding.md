...
Runs in the **browser**: bundled by `Bun.build` (with the React Compiler applied, see below) and embedded into the server binary as text. Type-checked by `packages/frontend/tsconfig.json` (lib `DOM` / `DOM.Iterable` + React types, plus `bun` types for `bun:test`).
...
- **The React Compiler runs at bundle time** (`packages/cli/src/react-compiler.ts`, Oxc's port): the shipped bundle auto-memoizes components and hooks, while `bun test` runs the uncompiled source — a panel change is only proven on compiled code in the browser. New code can rely on it instead of hand-memoizing; the existing `useCallback` / `useMemo` stay (the compiler replaces the calls with its own cache, and `react/preserve-manual-memoization` verifies the memoization they express survives). The `react/*` compiler rules in `.oxlintrc.json` are the pre-flight: a function that breaks them is skipped by the compiler silently, not reported. `"use no memo"` opts a function out.
...
- **Keep the bundle lean** — it's ~280 KB minified (mostly React). Don't add UI / util libraries.
