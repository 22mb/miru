...
- **stdout is machine output, stderr is for humans**: `miru review` prints the final JSON and `miru export` prints markdown to **stdout**; all logs / diagnostics go to `console.error`. Never `console.log` anything but the intended payload — the human↔AI loop parses stdout.
